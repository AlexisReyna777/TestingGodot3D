...
func _process(delta):
	if SPEED == SPRINT_SPEED:
		sprint_slider.value = sprint_slider.value - drain_amount * delta
		if sprint_slider.value == sprint_slider.min_value:
			SPEED = ORIGINAL_SPEED
	if SPEED != SPRINT_SPEED:
		if sprint_slider.value < sprint_slider.max_value:
			sprint_slider.value = sprint_slider.value + raise_amount * delta
		if sprint_slider.value == sprint_slider.max_value:
...
func _physics_process(delta):
	...
	var input_dir := Input.get_vector("left", "rigth", "fordward", "backward")
